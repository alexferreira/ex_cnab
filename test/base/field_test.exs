defmodule ExCnab.Test.Base.FieldTest do
  ...
  test "Do not: create field Why? Invalid formart" do
    ...
    assert {:error, _} = ExCnab.Base.Field.from_template(template, "something")
  end
  ...
  test "Do: create date field" do
    ...
    assert {:ok, _field} = ExCnab.Base.Field.enforce_format(field, date)
    ...
    assert {:ok, _field} = ExCnab.Base.Field.enforce_format(field_1, date_1)
  end
  ...
  test "Do: enforce field format" do
    ...
    Enum.map([{"date", date, len}, {"int", "#{int}", len}, {"decimal", "#{dec}", tp}, {"string", str, len}],
        fn {f, v, l} ->
            ...
            assert {:ok, _field} = ExCnab.Base.Field.enforce_format(field, v)
        end)
  end
end
